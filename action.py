...
#   - Paths are assumed beeing relative Accsyn default root share, as configured in Accsyn.
#   - Submits a transfer job and monitors it, through a job in Ftrack. 
#   - In Ftrack, removes the components from the destination location if they were present there.
#   - When job is done, adds them to the destination location in Ftrack.
#
# Part of Accsyn tutorial: Accsyn Ftrack Integration (https://support.accsyn.com/tutorial-ftrack-integration)
#
# Prerequisites:
# 
#   - Set environment variables FTRACK_SERVER, FTRACK_API_USER and FTRACK_API_KEY to authenticate Ftrack API prior to launch.
#   - Set environment variables ACCSYN_DOMAIN, ACCSYN_API_USER and ACCSYN_API_KEY to authenticate Accsyn API prior to launch.
#
            ...
            widgets = [
                {
                    'value':"This Action sends components from one location to another using Accsyn:<br><br>"+
                        "<ul>"+
                            ...
                            "<li>Includes paths containing project code (stripping path elements before project code), ignoring other paths.</li>"+
                            "<li>All paths are assumed beeing relative Accsyn default root share.</li>"
                        +"</ul>",
                    'type':"label"
                },
                {
    ...
    def threaded_run(self, event, entities):
        ...
        try:
            ...
            for component in all_components:
                ...
                # Create an Accsyn path from Ftrack path, modify your code here to identify and select different root shares if needed.
                # We are simply assuming project folders are named as Ftrack project code and resides directly beneath root path
                ...
                for p in values['additional_files'].split("\n"):
                    if p and 0<len(p):
                        ...
                        if 0<=idx:
                            ...
                            logging.warning("   Additional path '%s' could be evaluated, does not contain project code!"%(p))

            if len(components_and_paths) == 0:
                error("[ERROR] No components/additional files left after extracting paths!")
                job_final_status = "failed"
                return

            info("Building Accsyn job out of %d component(s)/files(s).."%len(components_and_paths))
            ...
            for (component, p, p_raw) in components_and_paths:
                if component:
                    logging.info("   Adding component '%s'(%s), path: %s (raw: %s)"%(component['name'], component['id'], p, p_raw))
                else:
                    logging.info("   Adding additional file: %s"%(p))
                source_party = ("site=%s"%source_location_name) if source_location_name.find("@") == -1 else source_location_name
                destination_party = ("site=%s"%destination_location_name) if destination_location_name.find("@") == -1 else destination_location_name
                accsyn_job_data['tasks'].append({
                    'source':"%s:%s"%(source_party, p),
                    'destination':"%s"%(destination_party)
                })
                component_count += 1
            ...
            info("Submitted (id: %s), removing component from destination location..."%(j['id']))
            for (component,p,p_raw) in components_and_paths:
                if component:
                    try:
                        cl = session.query('ComponentLocation where component.id="{0}" and location.id="{1}"'.format(component['id'],destination_location['id'])).first()
                        if cl:
                            info("(Post)    Removing component %s from location %s..."%(component, destination_location_name))
                            session.delete(cl)
                        session.commit()
                    except:
            ...
            if job_final_status == "done":
                info("(Post) Adding components to destination location: %s..."%(destination_location_name))
                for (component,p,p_raw) in components_and_paths:
                    if component:
                        try:
                            cl = session.query('ComponentLocation where component.id="{0}" and location.id="{1}"'.format(component['id'],destination_location['id'])).first()
                            ...
                            info("(Post)    Adding component %s to location %s @ path '%s'..."%(component, destination_location_name, p_raw))
                            session.commit()
                        except:
                            info(traceback.format_exc())
            info("(Post) Done...")
